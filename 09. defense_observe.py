...
Filename:
    ztd_09_defense_observe.py

Project:
    Zero Trust Desktop (ZTD)

Stage:
    09 — Defense + Observation Toolkit

Purpose
-------

Stage 09 installs baseline defense tooling plus observation / visibility
...
When run with no opt-in flags:
    ...
    4. Installs missing observation packages
    5. Captures evidence snapshots
    6. Writes JSONL audit logs
...
Logs:
    ~/.local/state/zero-trust-desktop/ztd_09/log/

Snapshots:
    ~/.local/state/zero-trust-desktop/ztd_09/snapshots/<timestamp>/

Evidence Bundle:
    ~/.local/state/zero-trust-desktop/ztd_09/snapshots/ztd_09_evidence_<timestamp>.tar.gz
...
APP_ID = "ztd"
STAGE_NAME = "09. DEFENSE + OBSERVATION TOOLKIT"
STAGE_ID = "ztd_09_defense_observe"
VERSION = "1.2.1"
...
def run(
    ...
    env = apt_env() if use_noninteractive_env else None
...
def snapshot_apt_source_state(s: Settings) -> None:
    ...
    safe_snapshot_command(
        s,
        "apt_policy_head.txt",
        ["bash", "-lc", "apt-cache policy 2>/dev/null | sed -n '1,220p' || true"],
        timeout=60,
    )
    ...
    if have("journalctl"):
        safe_snapshot_command(
            ...
            [
                "bash",
                "-lc",
                "journalctl -b --no-pager | egrep -i 'ssh|sudo|polkit|authentication failure|fail2ban|apparmor' | tail -n 240 || true",
            ],
            timeout=120,
        )
...
def run_clamav_scan(s: Settings) -> None:
    ...
    out = cap(["sudo", "clamscan", "-r", "-i", "--", str(target)], timeout=None)
...
def run_rkhunter_check(s: Settings) -> None:
    ...
    out = cap(["bash", "-lc", "sudo rkhunter --check --sk 2>&1 || true"], timeout=None)
    write_snapshot("rkhunter.txt", out + "\n", s.snapshot_root)


def run_chkrootkit_check(s: Settings) -> None:
    ...
    out = cap(["bash", "-lc", "sudo chkrootkit 2>&1 || true"], timeout=None)
    write_snapshot("chkrootkit.txt", out + "\n", s.snapshot_root)


def run_debsums_check(s: Settings) -> None:
    ...
    out = cap(["bash", "-lc", "sudo debsums -s 2>&1 || true"], timeout=None)
    write_snapshot("debsums.txt", out + "\n", s.snapshot_root)


def init_aide_database(s: Settings) -> None:
    if not s.init_aide_db:
        return

    aide_cmd = None
    if have("aideinit"):
        aide_cmd = "sudo aideinit 2>&1 || true"
    elif have("aide"):
        aide_cmd = "sudo aide --init 2>&1 || true"
    ...
    out = cap(["bash", "-lc", aide_cmd], timeout=None)
...
def bundle_evidence(s: Settings) -> Optional[Path]:
    ...
    out = s.snapshot_root.parent / f"ztd_09_evidence_{RUN_ID}.tar.gz"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="ztd_09_defense_observe.py")
...
def main() -> int:
    ...
    try:
        ...
        info(s, "[2] install defense packages (safer per-package mode)")
        defense_result = {"installed": [], "failed": []}
        for pkg in PKGS_DEFENSE:
            result = install_packages_with_fallback(s, f"defense:{pkg}", [pkg])
            defense_result["installed"].extend(result["installed"])
            defense_result["failed"].extend(result["failed"])
        ...
        info(s, "[7] optional scans (explicit only)")
        run_clamav_scan(s)
        run_rkhunter_check(s)
        run_chkrootkit_check(s)
        run_debsums_check(s)
        init_aide_database(s)
...
#
# MAKE EXECUTABLE
#   chmod +x ztd_09_defense_observe.py
#
# RUN (SAFE DEFAULT)
#   python3 ztd_09_defense_observe.py --yes
#
# RUN WITHOUT BANNER
#   python3 ztd_09_defense_observe.py --yes --no-banner
#
# RUN WITH EVIDENCE BUNDLE
#   python3 ztd_09_defense_observe.py --yes --bundle-evidence
#
# RUN WITH SAFE CLEANING
#   python3 ztd_09_defense_observe.py --yes --clean-safe
#
# INSTALL WIRESHARK GUI EXPLICITLY
#   python3 ztd_09_defense_observe.py --yes --install-wireshark-gui
#
# ENABLE WIRESHARK CAPTURE FOR CURRENT USER
#   python3 ztd_09_defense_observe.py --yes --enable-wireshark-capture
#
# RUN CLAMAV AGAINST HOME
#   python3 ztd_09_defense_observe.py --yes --run-clamav-scan "$HOME"
#
# RUN ROOTKIT / INTEGRITY CHECKS
#   python3 ztd_09_defense_observe.py \
#       --yes \
#       --run-rkhunter \
#       --run-chkrootkit \
#       --run-debsums
...
#   - Defense packages are installed per-package to reduce failure blast radius.
