...
def sudo(
    ...
    return run(
        ...
        check=check,
        timeout=timeout,
        use_noninteractive_env=use_noninteractive_env,
    )
...
def apt_update(s: Settings) -> None:
    try:
        ...
        if "Could not get lock" in stderr or "Unable to acquire the dpkg frontend lock" in stderr:
            error(
                s,
...
def run_clamav_scan(s: Settings) -> None:
    ...
    target = str(Path(s.run_clamav_path).expanduser())
    info(s, "Running clamscan (opt-in)", {"path": target})
    out = cap(["bash", "-lc", f"sudo clamscan -r -i -- '{target}' 2>&1 || true"], timeout=None)
...
#   - If grouped package install fails, the script retries package-by-package.
